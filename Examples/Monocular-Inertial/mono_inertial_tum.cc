...
int main(int argc, char **argv)
{
    ...
    for (seq = 0; seq<num_seq; seq++)
    {
        ...
        LoadImages(strFile, vstrImageFilenames[0], vTimestampsCam[0]);
        cout << "LOADED Yay!" << endl;
    ...
    for (seq = 0; seq<num_seq; seq++)
    {
        ...
        for(int ni=0; ni<nImages[seq]; ni++, proccIm++)
        {
            ...
            im = cv::imread(string(argv[3])+"/"+vstrImageFilenames[seq][ni],cv::IMREAD_UNCHANGED); //,cv::IMREAD_UNCHANGED);
    ...
    const string kf_file =  "kf_0.txt";
    const string f_file =  "f_0.txt";
