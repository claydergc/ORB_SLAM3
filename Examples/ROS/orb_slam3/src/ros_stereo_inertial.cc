...
int main(int argc, char **argv) {
  ...
  igb.mpSLAM->SaveTrajectoryTUM("/home/ros-noetic/src/ORB_SLAM3/A6_trajectory.txt");
...
void ImageGrabber::GrabImageLeft(const sensor_msgs::ImageConstPtr &img_msg) {
  ...
  ROS_INFO("time: %f", img_msg->header.stamp.toSec());
  ...
  while (ros::ok()) {
    ...
    if (!imgLeftBuf.empty() && !imgRightBuf.empty() &&
      ...
      for (int i = 0; i < keypoints_current_frame.size(); ++i) {
        ...
        //if (mvbVO[i]) { //I think that mvbVO is almost always false
      ...
      msg.header.stamp = ros::Time(currentFrame.mTimeStamp);//ros::Time::now();
      msg.header.frame_id = "map"; // Change as needed
      ...
      // if (true) {
      //  publish POSE 3D
      pose_msg.header.stamp = ros::Time(currentFrame.mTimeStamp);//ros::Time::now();
      ...
      if (last_keyframe != NULL) {
        ...
        std::vector<cv::KeyPoint> keypoints_last_keyframe =
            last_keyframe->mvKeys;
