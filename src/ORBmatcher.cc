  ...
  while (f1it != f1end && f2it != f2end) {
    if (f1it->first == f2it->first) {
      for (size_t i1 = 0, iend1 = f1it->second.size(); i1 < iend1; i1++) {
        ...
        const cv::KeyPoint &kp1 = (pKF1->NLeft == -1) ? pKF1->mvKeysUnNormalcam[idx1]
                                  : (idx1 < pKF1->NLeft)
                                      ? pKF1->mvKeysNormalcam[idx1]
                                      : pKF1->mvKeysRight[idx1 - pKF1->NLeft];

        const bool bRight1 =
            (pKF1->NLeft == -1 || idx1 < pKF1->NLeft) ? false : true;

        const cv::Mat &d1 = pKF1->mDescriptorsNormalcam.row(idx1);
        ...
        for (size_t i2 = 0, iend2 = f2it->second.size(); i2 < iend2; i2++) {
          ...
          const cv::Mat &d2 = pKF2->mDescriptorsNormalcam.row(idx2);
...
int ORBmatcher::SearchForTriangulationPolcam(
    KeyFrame *pKF1, KeyFrame *pKF2, vector<pair<size_t, size_t>> &vMatchedPairs,
    const bool bOnlyStereo, const bool bCoarse) {
  const DBoW2::FeatureVector &vFeatVec1 = pKF1->mFeatVecPolcam;
  const DBoW2::FeatureVector &vFeatVec2 = pKF2->mFeatVecPolcam;
  ...
  // Compare only ORB that share the same node
  int nmatches = 0;
  vector<bool> vbMatched2(pKF2->NPolcam, false);
  vector<int> vMatches12(pKF1->NPolcam, -1);
  ...
  while (f1it != f1end && f2it != f2end) {
    if (f1it->first == f2it->first) {
      for (size_t i1 = 0, iend1 = f1it->second.size(); i1 < iend1; i1++) {
        ...
        const cv::KeyPoint &kp1 = (pKF1->NLeft == -1) ? pKF1->mvKeysUnPolcam[idx1]
                                  : (idx1 < pKF1->NLeft)
                                      ? pKF1->mvKeysPolcamNonOverlapped[idx1]
                                      : pKF1->mvKeysRight[idx1 - pKF1->NLeft];

        const bool bRight1 =
            (pKF1->NLeft == -1 || idx1 < pKF1->NLeft) ? false : true;

        const cv::Mat &d1 = pKF1->mDescriptorsPolcamNonOverlapped.row(idx1);
        ...
        for (size_t i2 = 0, iend2 = f2it->second.size(); i2 < iend2; i2++) {
          ...
          const cv::Mat &d2 = pKF2->mDescriptorsPolcamNonOverlapped.row(idx2);
          ...
          const cv::KeyPoint &kp2 = (pKF2->NLeft == -1) ? pKF2->mvKeysUnPolcam[idx2]
                                    : (idx2 < pKF2->NLeft)
        ...
        if (bestIdx2 >= 0) {
          const cv::KeyPoint &kp2 =
              (pKF2->NLeft == -1) ? pKF2->mvKeysUnPolcam[bestIdx2]
              : (bestIdx2 < pKF2->NLeft)
                  ? pKF2->mvKeysPolcamNonOverlapped[bestIdx2]
                  : pKF2->mvKeysRight[bestIdx2 - pKF2->NLeft];
          vMatches12[idx1] = bestIdx2;
          nmatches++;
...
int ORBmatcher::SearchByProjectionPolcam(Frame &CurrentFrame, const Frame &LastFrame,
                                   const float th, const bool bMono) {
  // int ORBmatcher::SearchByProjection(Frame &CurrentFrame, const Frame
  // &LastFrame,
  //                                    const float th, const bool bMono,
  ...
  //std::cout<<"LastFrame.NPolcam: "<<LastFrame.NPolcam<<std::endl;
  //std::cout<<"LastFrame.mvpMapPoints.size: "<<LastFrame.mvpMapPoints.size()<<std::endl;

  //for (int i = 0; i < LastFrame.NPolcam; i++) {
  for (int i = LastFrame.N; i < LastFrame.N_NPolcam; i++) {
    MapPoint *pMP = LastFrame.mvpMapPointsPolcam[i];
    if (pMP) {
      if (!LastFrame.mvbOutlier[i]) {
        // Project
        Eigen::Vector3f x3Dw = pMP->GetWorldPos();
        ...
        int nLastOctave = LastFrame.mvKeysPolcamNonOverlapped[i].octave;
            //(LastFrame.Nleft == -1 || i < LastFrame.Nleft)
            //    ? LastFrame.mvKeys[i].octave
            //    : LastFrame.mvKeysRight[i - LastFrame.Nleft].octave;
        ...
        if (bForward)
          vIndices2 =
              CurrentFrame.GetFeaturesInAreaPolcam(uv(0), uv(1), radius, nLastOctave);
        else if (bBackward)
          vIndices2 = CurrentFrame.GetFeaturesInAreaPolcam(uv(0), uv(1), radius, 0,
                                                     nLastOctave);
        else
          vIndices2 = CurrentFrame.GetFeaturesInAreaPolcam(
              uv(0), uv(1), radius, nLastOctave - 1, nLastOctave + 1);
        ...
        for (vector<size_t>::const_iterator vit = vIndices2.begin(),
             ...
             vit != vend; vit++) {
          //const size_t i2 = *vit;
          const size_t i2 = CurrentFrame.N+*vit;
         ...
         // }
          ...
          const cv::Mat &d = CurrentFrame.mDescriptorsPolcamNonOverlapped.row(i2-CurrentFrame.N);
        ...
        if (bestDist <= TH_HIGH) {
          ...
          if (mbCheckOrientation) {
            cv::KeyPoint kpLF = LastFrame.mvKeysUnPolcam[i-LastFrame.N];                

            cv::KeyPoint kpCF = CurrentFrame.mvKeysUnPolcam[bestIdx2-CurrentFrame.N];
