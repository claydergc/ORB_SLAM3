...
void LocalMapping::Run() {
  ...
  while (1) {
    ...
    if (CheckNewKeyFrames() && !mbBadImu) {
      ...
      CreateNewMapPoints(); //With this function it fails as usually with polcamI. Definitely it fails with this!!!
      // CreateNewMapPointsMonoAndPolcam();


      //std::cout<<"mPolcamMode="<<mPolcamMode<<std::endl;
...
void LocalMapping::CreateNewMapPointsAllCams(bool cam0, bool cam1, bool cam2, bool cam3) {
  ...
  for (size_t i = 0; i < vpNeighKFs.size(); i++) {
    ...
    // std::cout<<"nmatches[0]="<<nmatches[0]<<std::endl;
    // std::cout<<"nmatches[1]="<<nmatches[1]<<std::endl;
...
void LocalMapping::setPolcamMode(Constants::ePolcamMode mode)
{
    mPolcamMode = mode;
}
